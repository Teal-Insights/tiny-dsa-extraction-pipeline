...
def compute_multi_member_cluster_refactor_order(
    projection: ProjectionResult,
    clusters: tuple[FormulaCluster, ...],
) -> tuple[FormulaCluster, ...]:
    """Return multi-member clusters in dependency order (dependencies first).

    Among clusters with no remaining inbound edges, ``cluster_id`` ascending
    breaks ties deterministically.
    """
    eligible = tuple(cluster for cluster in clusters if len(cluster.members) >= 2)
    if not eligible:
        return ()
