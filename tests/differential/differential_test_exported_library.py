...
MVP Oracle
    ...
    8464d45 / 9831beb / 78fd00c), loaded as ``dist.api``. Inputs are applied
    via the package's Records-shaped ``set_*`` functions; outputs are read
    from the Records-shaped ``compute_*`` functions. The package does not
...
def write_txt_summary(
    ...
    *,
    ...
    with path.open("w", encoding="utf-8") as f:
        ...
        f.write(f"Workbook:  {WORKBOOK_PATH}\n")
        f.write(
            f"Package:   {EXPORTED_PACKAGE_DIR} (imported as {EXPORTED_PACKAGE_NAME})\n"
        )
...
def _verify_paths() -> None:
    ...
    if (
        not (EXPORTED_PACKAGE_DIR / "__init__.py").is_file()
        or not (EXPORTED_PACKAGE_DIR / "api.py").is_file()
    ):
        raise FileNotFoundError(
            f"Exported package incomplete at {EXPORTED_PACKAGE_DIR} "
...
def _check_staleness() -> None:
    ...
    """
    data_path = EXPORTED_PACKAGE_DIR / "data.py"
...
def run_differential_test() -> int:
    """Run every scenario through both oracles; write the parity report.

    Returns
    -------
    int
        0 if every comparison passes at ``ATOL``, 1 otherwise.
    """
    _verify_paths()
    _check_staleness()
    api = load_exported_library(PROJECT_ROOT, EXPORTED_PACKAGE_NAME)
    ...
    for scenario in scenarios:
        try:
            excel_outputs = run_excel_oracle(WORKBOOK_PATH, scenario, output_addresses)
        ...
        comparisons.extend(
            compare_scenario(
                scenario, excel_outputs, mvp_outputs, cell_labels, atol=ATOL
            )
        )

    write_csv_report(comparisons, REPORT_DIR / "parity_report.csv")
    write_txt_summary(comparisons, REPORT_DIR / "parity_report.txt", atol=ATOL)
