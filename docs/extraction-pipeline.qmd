...
This rendering cell calls DeepSeek once per generated series API function.
...
Then we can use LLM calls to rewrite guide sections into Python-first user guide pages. We do one call per section, keep a cache, and pass only the context each section needs.
...
We will copy the first section of the guide, `I. Introduction`, into the documentation website's landing page, which must be named `index.qmd`.
