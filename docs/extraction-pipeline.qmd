...
Once that's done, we can build docs in two phases: first run `great-docs build` with freeze-only mode to avoid Quarto invoking system Python for post-render; then run the generated `post-render.py` directly under `uv` so enhancement transforms still apply.
