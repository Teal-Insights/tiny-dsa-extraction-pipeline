...
In the current architecture of `excel-grapher, constraints don't persist on the graph object, so we need to pass them to the `CodeGenerator` as a dictionary. (Persisting the constraints on the `DependencyGraph` object is planned for a future release and will obviate the need for this step.)
...
Then we define our entrypoints, which are the output functions we want to export. The keys of this dictionary are appended to `compute_` prefix to form the function names, and can be any arbitrary strings. The values are the list of cell addresses, range addresses, or named ranges that collectively comprise the output range that the function will compute and return.
...
Finally, we can generate the code and write it to a file, [dist/tiny_dsa.py](tiny_dsa.py).
