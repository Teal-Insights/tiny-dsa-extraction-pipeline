...
for one parameterized helper body per cluster, validate the response, apply Phase
A (helper insert plus thin wrappers), and **write the refactored source back to
`internals.py`**. That post-refactor file is what the differential suite exercises.
...
flowchart LR
  ...
  E --> F[apply_refactor_plan]
  F --> G[differential tests]
```
...
5. **Apply Phase A** — insert the helper and replace member bodies with thin
   wrappers (deterministic), then write `internals.py`.
6. **Test** — `ast.parse`, `compile`, then the exported-library differential script.

Phase B (optional direct-call substitution at `xl_eval` sites) is implemented
but not wired into the default apply path. Phase A alone preserves the entire
`_resolve_formula` / `xl_eval` machinery.
...
`call_sites`. After the full refactor pass, equivalent references often live
inside parameterized helpers such as `debt_to_gdp` instead, while Phase A leaves
the original `xl_eval(..., cell_engine_c10)` call sites in place.
...
only**—no LLM call and no file write. The next cell runs the full five-cluster
pass for real.
...
hundreds of lines of parallel logic.

#### Apply all clusters and write `internals.py`

`refactor_internals_all_clusters()` walks `REFACTOR_ROW_ORDER` — `(10, 16, 6,
20, 14)` — re-extracting `ClusterRefactorContext` after each apply so later LLM
calls see upstream wrappers. Responses come from `.cache/internals-refactors.json`
when available; uncached clusters call DeepSeek (requires `DEEPSEEK_API_KEY` in
`.env`). Each successful apply validates the response, runs Phase A, and writes
the updated source to `internals_path`.
...
codegen_size = internals_path.stat().st_size
...
#### Phase B — optional direct-call substitution

After Phase A passes differential tests, `rewrite_xl_eval_call_sites()` can
replace patterns like:

```python
xl_eval(ctx, 'Engine!C10', cell_engine_c10)
```
...
inside functions **outside** the member set. Member wrappers would remain as the
stable address→callable binding for `_resolve_formula` unless the resolver is
extended. **We ship Phase A only first**; Phase B is an optional size win.
