...
Then the document stage launches a local Cursor SDK agent against `dist/`
(see `src/documentation_pipeline.py`). The agent reads
`docs-source/guidance-note.md` (a copy of the Excel guidance note),
explores the generated package API, writes economist-facing Quarto pages
under `user_guide/` in whatever structure it finds most useful, and
renders with:
...
Authored trees are cached under `.cache/user-guide/<key>/`. Cache keys
fold the document agent model, prompt template version, guidance-note
bytes, and generated `api.py` text. On a cache miss the stage requires
`CURSOR_API_KEY`. Default model is `gpt-5.6-luna`
(`DOCUMENT_AGENT_MODEL`). Interrupted runs can resume via
`.cache/user-guide/active.json`.

After the agent (or cache restore), the pipeline:

1. Writes the deterministic `03-excel-parity-validation.qmd` page from
   the parity report.
2. Applies optional `RUNNABLE_CELL_RULES` to `{python}` cells.
3. Re-runs the same `great-docs build` command as a fail-closed gate.
4. Refuses the stage if the agent modified generated package modules or
   left `user_guide/` empty.
5. Writes the docs deploy workflow.
...
`refactor_internals_all_clusters()` refactors singletons first, then orders
multi-member clusters with `compute_multi_member_cluster_refactor_order()` from
`src/refactor_order.py`. It re-extracts context after each collapse so later LLM
calls see already-collapsed upstream helpers.
