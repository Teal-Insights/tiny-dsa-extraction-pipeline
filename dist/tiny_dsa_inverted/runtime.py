...
Mechanical extraction should emit calls to these helpers instead of reading
cells from an evaluation context. ``trim`` is the only way a caller shortens a
...
def xl_match(
    lookup: object, lookup_array: Sequence[object], match_type: int = 0
) -> int:
...
def trim[T](values: Sequence[T], stop: int, *, start: int = 0) -> tuple[T, ...]:
    ...
    if not (0 <= start <= stop <= length):
        raise ValueError(
            f"trim slice [{start}:{stop}] is outside series of length {length}"
        )
